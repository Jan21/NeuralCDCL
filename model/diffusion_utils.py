...
def prepare_diffusion(batch, diffusion,num_vars):
    b,t = batch['labels'].shape
    pi = torch.ones(b)*num_vars
    point_indicator = pi.int()
    t = np.random.randint(1, diffusion.T + 1, b).astype(int)
