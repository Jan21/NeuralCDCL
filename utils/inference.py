...
def calculate_metrics(results_dict, tokenizer):
    overall_metrics = {
        ...
        'assigned_token_accuracy': [],
        'conflict_token_accuracy': []
    }
    
    dataset_metrics = {}
    
    # Get token IDs for the key phrases
    variable_id = tokenizer.encode("variable", add_special_tokens=False)[0]
    assigned_id = tokenizer.encode("assigned:", add_special_tokens=False)[0]
    found_id = tokenizer.encode("found", add_special_tokens=False)[0]
    conflict_id = tokenizer.encode("conflict:", add_special_tokens=False)[0]
    ...
    for datapath, data in results_dict.items():
        ...
        for i in range(len(gt_solutions_ids)):
            ...
            # Extract full variable assignments from ground truth
            gt_var_assignments = extract_patterns(gt_ids, [variable_id, assigned_id], semicolon_id)
            
            # Extract full variable assignments from prediction
            pred_var_assignments = extract_patterns(pred_ids, [variable_id, assigned_id], semicolon_id)
            
            # Calculate variable assignment accuracy based on pattern matching
            var_assigned_acc = calculate_pattern_accuracy(gt_var_assignments, pred_var_assignments)
            
            # Extract full conflict patterns from ground truth
            gt_conflicts = extract_patterns(gt_ids, [found_id, conflict_id], semicolon_id)
            
            # Extract full conflict patterns from prediction
            pred_conflicts = extract_patterns(pred_ids, [found_id, conflict_id], semicolon_id)
            
            # Calculate conflict accuracy based on pattern matching
            found_conflict_acc = calculate_pattern_accuracy(gt_conflicts, pred_conflicts)
            ...
            dataset_metrics[datapath]['assigned_token_accuracy'].append(var_assigned_acc)
            dataset_metrics[datapath]['conflict_token_accuracy'].append(found_conflict_acc)
            ...
            overall_metrics['assigned_token_accuracy'].append(var_assigned_acc)
            overall_metrics['conflict_token_accuracy'].append(found_conflict_acc)
...
def extract_patterns(token_ids, start_sequence, delimiter_token):
    """
    Extract complete patterns from token IDs between start_sequence and delimiter_token.
    
    Args:
        token_ids: List of token IDs
        start_sequence: Sequence of token IDs to identify the start of a pattern
        delimiter_token: Token ID that marks the end of a pattern
    
    Returns:
        List of extracted patterns (each pattern is a list of token IDs)
    """
    patterns = []
    
    # Iterate through the token IDs
    for i in range(len(token_ids) - len(start_sequence) + 1):
        # Check if we found the start sequence
        if token_ids[i:i+len(start_sequence)] == start_sequence:
            # Find the next delimiter (semicolon)
            pattern_start = i
            pattern_end = None
            
            for j in range(i + len(start_sequence), len(token_ids)):
                if token_ids[j] == delimiter_token:
                    pattern_end = j
                    break
            
            # If we found a complete pattern
            if pattern_end is not None:
                # Extract the complete pattern (including start sequence and up to semicolon)
                pattern = token_ids[pattern_start:pattern_end]
                patterns.append(pattern)
    
    return patterns

def calculate_pattern_accuracy(gt_patterns, pred_patterns):
    """
    Calculate accuracy between ground truth patterns and predicted patterns.
    
    Args:
        gt_patterns: List of ground truth patterns
        pred_patterns: List of predicted patterns
    
    Returns:
        Accuracy score between 0.0 and 1.0
    """
    if not gt_patterns and not pred_patterns:
        return 1.0  # Both have zero patterns, perfect match
    
    if not gt_patterns or not pred_patterns:
        return 0.0  # One has patterns, the other doesn't
    
    # Convert patterns to strings for comparison (using string representation of token IDs)
    gt_pattern_strs = ['_'.join(map(str, pattern)) for pattern in gt_patterns]
    pred_pattern_strs = ['_'.join(map(str, pattern)) for pattern in pred_patterns]
    
    # Count matches (exact pattern matches)
    matches = 0
    for gt_pattern in gt_pattern_strs:
        if gt_pattern in pred_pattern_strs:
            matches += 1
            # Remove the match to avoid double counting
            pred_pattern_strs.remove(gt_pattern)
    
    # Accuracy is the number of matches divided by the total patterns
    return matches / max(len(gt_patterns), len(pred_patterns))
